...
variable queue = <<>>; \*sintassi per lista vuota
define
  BoundedQueue == Len(queue) <= MaxQueueSize \*boundedBuffer.
end define;
...
begin Produce:
  while TRUE do
    produce:
        item := "item";
    put: 
        await Len(queue) < MaxQueueSize; \*aspetta che ci sia almeno un posto. Poi la inserisce
...
begin Consume:
  while TRUE do
    take: \*aspetta che ci sia un element.
        await queue /= <<>>;
        item := Head(queue); \*da l'elemento di testa della lista e rimuoverlo.
        queue := Tail(queue); \*toglie l'elemento dalla coda.
    consume:
        print item;
  end while;
end process;
...
\* Last modified Mon Apr 03 15:24:33 CEST 2023 by Dach-
